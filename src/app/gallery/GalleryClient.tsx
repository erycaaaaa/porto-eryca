...
const CATEGORIES: Category[] = ["All", "Acrylic", "Watercolor", "3D Crafting", "Poster", "Sketch", "Design"];
...
export default function GalleryClient({
  ...
  const filtered = useMemo(() => {
    return items.filter((it) => {
      const byCat = active === "All" || it.category === active;
      const byQ = !q || it.title.toLowerCase().includes(q.toLowerCase());
      return byCat && byQ;
    });
  }, [items, active, q]);
